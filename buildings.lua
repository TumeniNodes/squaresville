		...
		for z = 0,dz+1 do
			for x = 0,dx+1 do
				...
				if (x == 0 or x == dx + 1) and z % 5 == 4 then
					data[x][y][z] = node("squaresville:gargoyle")
			...
			for y = 0,(floors * 4) do
				if y % 4 == 0 and x > 2 and z > 2 and x < dx - 1 and z < dz - 1 then
					if floors * 4 - y < 4 then
						data[x][y][z] = node(breaker("squaresville:roof"))
					else
						data[x][y][z] = node(breaker("squaresville:floor_ceiling"))
					end
				elseif wall_x then
					if y == 0 then
				...
				elseif wall_x_2 and develop then
					...
					elseif y % 4 ~= 2 or z % 5 == 4 then
						...
						data[x][y][z] = node(breaker("squaresville:plate_glass"))
					end
				elseif wall_z then
					if y == 0 then
						data[x][y][z] = node(breaker(conc))
  ...
  for bz = minp.z - 2 * block_size + 1, maxp.z + block_size - 1 do
    for bx = minp.x - 2 * block_size + 1, maxp.x + block_size - 1 do
      for non_loop = 1, 1 do
        ...
        if sr <= 3 then
          gotham(write, dx, dy, dz)
        elseif sr <= 6 then
          glass_and_steel(write, dx, dy, dz)
        elseif sr <= 9 then
          clear(1, 5)
          simple(write, size)
        elseif sr <= 12 then
